...
path='F:/paper/desp/'
testpath = 'F:/paper/test/'
def tain_classify(filename,dicname):
    #如果是excel另存为csv 则需要修改读取方式r为wU
    predict = Predict()
    testfile = file(testpath+dicname+filename, 'rU')
    ...
    if( tt==0 or tt==len(predict.test_data)):
        print 'None'
        os.remove(os.path.join(testpath, dicname+filename))
        return None
    
    if not os.path.isfile(path+dicname+filename):
        return None
    datafile = file(path+dicname+filename, 'rU')
...
if __name__ == "__main__":
    for i in range(3,11):
        filelist = os.listdir(testpath+str(i)+'comp/')
        outputf = codecs.open('output_test_svm'+str(i)+'.txt','w','utf-8')#open for 'w'riting
        for data in filelist:
    #         print file;
            result = tain_classify(data,str(i)+'comp/')
